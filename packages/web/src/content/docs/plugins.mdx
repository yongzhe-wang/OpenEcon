---
title: Plugins
description: Write your own plugins to extend OpenEcon.
---

Plugins allow you to extend OpenEcon by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify OpenEcon's default behavior.
...
OpenEcon runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the OpenEcon desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://openecon.ai/docs/sdk) for details.
