---
title: TUI
description: Using the OpenEcon terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenEcon provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenEcon starts the TUI for the current directory.
...
When using the OpenEcon TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenEcon. Allows you to select from available providers and add their API keys.
...
Exit OpenEcon. _Aliases_: `/quit`, `/q`
