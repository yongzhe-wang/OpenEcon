---
title: Providers
description: Using any LLM provider in OpenEcon.
---

import config from "../../../config.mjs"
export const console = config.console

OpenEcon uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenEcon config.
...
You can customize the providers through the `provider` section in your OpenEcon
config.
...
OpenEcon Zen is a list of models provided by the OpenEcon team that have been
tested and verified to work well with OpenEcon. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenEcon Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenEcon Zen`, and head to [openecon.ai/auth](https://openecon.ai/zen).
...
It works like any other provider in OpenEcon and is completely optional to use.

---

## OpenEcon Go

OpenEcon Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the OpenEcon team that have been
tested and verified to work well with OpenEcon.

1. Run the `/connect` command in the TUI, select `OpenEcon Go`, and head to [openecon.ai/auth](https://openecon.ai/zen).
...
To use Amazon Bedrock with OpenEcon:
...
OpenEcon. Anthropic explicitly prohibits this.

Previous versions of OpenEcon came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in OpenEcon with zero setup:
...
OpenEcon uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenEcon
...
To use Google Vertex AI with OpenEcon:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each OpenEcon conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenEcon conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for OpenEcon. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with OpenEcon:
...
3. Copy the API key for use in OpenEcon.
...
6. **Important**: Before using cloud models in OpenEcon, you must pull the model information locally:
...
OpenEcon Zen is a list of tested and verified models provided by the OpenEcon team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenEcon Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenEcon Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenEcon API key.
...
The `limit` fields allow OpenEcon to understand how much context you have left. Standard providers pull these from models.dev automatically.
